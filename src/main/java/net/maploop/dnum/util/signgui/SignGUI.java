...
public class SignGUI {
	...
	public SignGUI(Player player, String[] lines, UpdateHandler uh) {
		...
		Bukkit.getScheduler().runTaskLater(Dnum.getInstance(), () ->
		{
			try {
		...
		Bukkit.getPluginManager().registerEvents(listener, Dnum.getInstance());
	...
	private void registerSignUpdateListener(UpdateHandler uh) {
		...
		packetListener = new PacketAdapter(Dnum.getInstance(), PacketType.Play.Client.UPDATE_SIGN) {
			@Override
			public void onPacketReceiving(PacketEvent event) {
				if (event.getPlayer().equals(player)) {
					...
					Bukkit.getScheduler().runTask(Dnum.getInstance(), () -> sign.getBlock().setType(Material.AIR));
